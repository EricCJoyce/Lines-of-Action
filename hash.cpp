...
static const unsigned long long zobristHashTable[] = {9500759203796429788,
                                                      4154365239631495907,
    ...
    for(y = 0; y < 8; y++)                                          //  (8 bytes) Decode white.
      {
        ...
        for(x = 0; x < 8; x++)
          {
            if((ch & mask) == mask)
              h ^= zobristHashTable[ W_A1 + (y * 8 + x) ];
            mask >>= 1;
          }
      }

    if((hashInputBuffer[i] & 128) == 128)                           //  Hash the side to move.
      h ^= zobristHashTable[ B_TO_MOVE ];
