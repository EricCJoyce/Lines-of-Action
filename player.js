            ...
            WebAssembly.instantiate(bytes,
              {
                env: {
                       memoryBase: 0,
                       tableBase: 0,
                                                                    //  Malloc 1 page for 15.2 KB file.
                    ...
                    WebAssembly.instantiate(bytes,
                      {
                        env: {
                               memoryBase: 0,
                               tableBase: 0,
                                                                    //  Compiled with -s INITIAL_MEMORY=14745600 = 225 pages.
                               memory: new WebAssembly.Memory({initial: 225}),
    ...
    lookup()
      {
        if(!this.callOut)
          {
            var params = 'sendRequest=gropius';
            var i;
            for(i = 0; i < _GAMESTATE_BYTE_SIZE; i++)
                ...
                if(this.readyState == 4 && this.status == 200)
                  {
                    if(this.responseText == "")                     //  Null return: unknown error. Whatever; proceed with search.
                    ...
                    else
                      {
                        ...
                        if(parse[0] == 'linesofaction')             //  Contact!
                          {
