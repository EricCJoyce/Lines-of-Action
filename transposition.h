...
#define _TRANSPO_RECORD_BYTE_SIZE     17                            /* Number of bytes needed to store a TranspoRecord object. */
...
typedef struct TranspoRecordType                                    //  TOTAL: 17 = _TRANSPO_RECORD_BYTE_SIZE
  {
    ...
    unsigned char bestMove[_MOVE_BYTE_SIZE];                        //  (2 bytes) The best move for this record, stored as a byte array.
    unsigned char depth;                                            //  (1 byte)  Search depth ratifying this record.
